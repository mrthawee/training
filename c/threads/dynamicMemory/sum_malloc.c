...
//long long sum = 0; -> We don't wanna use Global, let's use malloc()
...
void* sum_runner(void *arg)
{
   ...
   *answer = sum;
   ...
   pthread_exit(answer); // someone have to free it later!
}
...
//   long long limit = atoll(argv[1]);
   ...
   *limit = atoll(argv[1]);
   ...
   printf("Sum is %lld\n", *result);
   free(result);  // this one was malloc by the thread.
